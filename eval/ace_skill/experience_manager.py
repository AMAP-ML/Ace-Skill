...
class ExperienceMemoryProvider:
    ...
    def __init__(self, experiences: Dict[str, str], retriever: Optional[ExperienceRetriever] = None):
        ...
        # Internal embedding cache: text_hash -> np.ndarray
        # Persists across multiple _embed_texts / _search_similar calls so that
        # only genuinely new texts hit the embedding API.
        self._embedding_cache: Dict[str, np.ndarray] = {}

        # Pre-populate cache from retriever's already-computed embeddings
        if retriever is not None and hasattr(retriever, '_experience_embeddings'):
            for exp_id, emb in retriever._experience_embeddings.items():
                if exp_id in retriever.experiences:
                    text_hash = self._text_hash(retriever.experiences[exp_id])
    ...
    def _text_hash(text: str) -> str:
        """Compute hash of text for content-addressed caching."""
    ...
    def _embed_texts(
        ...
        """Generate embeddings for texts, using internal cache to skip redundant API calls."""
        if not texts:
        ...
        for i, text in enumerate(texts):
            ...
            if th in self._embedding_cache:
                results[i] = self._embedding_cache[th]
            else:
        ...
        if texts_to_embed:
            ...
            new_embeddings = retriever._generate_embeddings_batch(texts_to_embed, batch_size=batch_size)
            for j, idx in enumerate(embed_indices):
                emb = new_embeddings[j] if j < len(new_embeddings) else None
                if emb is not None:
                    results[idx] = emb
                    self._embedding_cache[self._text_hash(texts_to_embed[j])] = emb
...
def _reduce_experiences_to_limit(
    ...
    while len(current) > limit and round_idx < max_rounds:
        ...
        try:
            ...
            # provider.experiences is already updated by add/remove calls above;
            # reuse the same provider to preserve its _embedding_cache.
...
def refine_experience_library(
    ...
    """
    Perform a library-wide refinement pass on the entire experience library.
