"""
CodeInterpreter Tool - Execute Python code in a stateful Jupyter kernel.
"""

import asyncio
import ast
import atexit
import base64
import concurrent.futures
import io
import json
import os
import queue
import re
import signal
import subprocess
import sys
import threading
import time
import uuid
from typing import Dict, Set

from PIL import Image
from jupyter_client import BlockingKernelClient
from tools.base import BaseTool
from tools.tool_registry import register_tool

# Constants
IMAGE_EXTENSIONS = ('.png', '.jpg', '.jpeg', '.gif', '.bmp')
KERNEL_INIT_POLL_INTERVAL = 0.1  # seconds
KERNEL_INIT_MAX_WAIT = 10  # seconds
KERNEL_INIT_SLEEP = 0.1  # seconds after initialization

# Maximum number of live kernel processes allowed system-wide.
# When the limit is reached, the oldest kernels (by creation time) are evicted
# before a new one is started.  This prevents unbounded accumulation of kernel
# processes across rollouts and threads.
_MAX_ACTIVE_KERNELS = 8

# Global variables for kernel management
_KERNEL_CLIENTS: dict = {}
_MISC_SUBPROCESSES: Dict[str, subprocess.Popen] = {}
_KERNEL_CREATION_TIME: Dict[str, float] = {}
_KERNEL_LOCK = threading.Lock()  # Thread lock to protect global dictionaries

# Limit concurrent kernel startups to avoid system overload when many threads
# try to launch kernels simultaneously.  Only one kernel initializes at a time;
# others queue up and wait.  The value 1 is intentionally conservative – kernel
# startup is a short, resource-intensive burst and serializing it avoids the
# scenario where N threads each spawn a kernel process that never becomes ready
# because the CPU is saturated by the other N-1 startup attempts.
_KERNEL_STARTUP_SEMAPHORE = threading.Semaphore(1)

LAUNCH_KERNEL_PY = """
from ipykernel import kernelapp as app
app.launch_new_instance()
"""


def _kill_kernels_and_subprocesses(_sig_num=None, _frame=None):
    """Clean up all kernels and subprocesses."""
    with _KERNEL_LOCK:
        for v in list(_KERNEL_CLIENTS.values()):
            try:
                v.shutdown()
            except:
                pass
        _KERNEL_CLIENTS.clear()
        
        for v in list(_MISC_SUBPROCESSES.values()):
            try:
                v.terminate()
            except:
                pass
        # Give processes a moment to exit gracefully, then force-kill survivors
        import time as _time
        _time.sleep(0.5)
        for v in list(_MISC_SUBPROCESSES.values()):
            if v.poll() is None:
                try:
                    v.kill()
                    v.wait(timeout=5)
                except:
                    pass
            else:
                try:
                    v.wait(timeout=1)
                except:
                    pass
        _MISC_SUBPROCESSES.clear()
        _KERNEL_CREATION_TIME.clear()


# Register cleanup function
if threading.current_thread() is threading.main_thread():
    atexit.register(_kill_kernels_and_subprocesses)


def execute_with_timeout(func, timeout, **kwargs):
    """Execute a function with timeout support and graceful interruption.
    
    Note: Python threads cannot be truly killed, so after timeout:
    1. Set stop_event to notify the function to exit
    2. Use wait=False to avoid blocking
    3. Caller must ensure kernel process is forcefully terminated
    """
    import time as time_module
    import threading
    start_time = time_module.time()
    
    # Create stop event for graceful interruption
    stop_event = threading.Event()
    kwargs['_stop_event'] = stop_event
    
    # Don't use with statement to avoid waiting for task completion on exit
    executor = concurrent.futures.ThreadPoolExecutor(max_workers=1)
    try:
        future = executor.submit(func, **kwargs)
        try:
            result = future.result(timeout=timeout)
            elapsed = time_module.time() - start_time
            print(f"[CodeInterpreter] execute_with_timeout completed in {elapsed:.2f}s (timeout was {timeout}s)")
            return result
        except concurrent.futures.TimeoutError:
            # Set stop event to notify function to exit early
            stop_event.set()
            # Try to cancel task (may be ineffective for already running tasks, but at least marks as cancelled)
            future.cancel()
            elapsed = time_module.time() - start_time
            print(f"[CodeInterpreter] Execution timeout after {elapsed:.2f}s (limit: {timeout}s), stop event set")
            return f'TimeoutError: Function execution timed out after {timeout} seconds.'
        except Exception as e:
            # Also set stop event when exception occurs
            stop_event.set()
            elapsed = time_module.time() - start_time
            print(f"[CodeInterpreter] execute_with_timeout exception after {elapsed:.2f}s: {e}")
            return f'Fatal Error: {e}'
    finally:
        # Use wait=False to return immediately, don't wait for timed-out tasks to complete
        # Timed-out tasks will continue running in the background until completed or terminated by kernel
        executor.shutdown(wait=False)


@register_tool('code_interpreter')
class CodeInterpreter(BaseTool):
    name = 'code_interpreter'
    description = '''
Executes Python code in a stateful Jupyter kernel and return results.

Capabilities:
- Use PIL, NumPy, or OpenCV to process or analyze images to improve understanding.
- Perform complex mathematical calculations or data manipulation.
- Code execution is persistent: variables and functions are stored and reusable in subsequent calls.

Image Variables (Pre-loaded as PIL Image objects):
- original_image / original_image_N (N=1,2,3,...): The original input images are already loaded as PIL Image objects. The following code is automatically executed before your code:
  ```python
  from PIL import Image
  original_image = Image.open('...')
  original_image_1 = Image.open('...')  # if multiple images exist
  ```
  You can directly use these variables (e.g., `original_image`, `original_image_1`) without loading them again.
- tool_image_N (N=1,2,3,...): Images generated by your previous code calls.
- observation_N (N=1,2,3,...): Images generated by zoom tool operations.

Visualization & Output:
- To view generated images, return the object, use plt.show(), or save it (e.g., image.save('out.png')). 
- DO NOT use image.show(). 
- Displayed images will be available as "tool_image_N" in the next turn.

Pre-installed packages:
- PIL
- NumPy
- OpenCV
- Matplotlib
- SciPy
- Scikit-learn
- Scikit-image
- Pandas
- SymPy
'''
    parameters = {
        "type": "object",
        "properties": {
            "code": {
                "type": "string",
                "description": "Python code to execute in the Jupyter kernel"
            }
        },
        "required": ["code"]
    }
    
    def __init__(self, config=None):
        super().__init__(config)
        # Instance ID (generated before setting work directory)
        self.instance_id = str(uuid.uuid4())
        # Track all kernel IDs registered by this instance so __del__/cleanup()
        # can clean them up without relying on threading.get_ident().
        self._registered_kernel_ids: set = set()
        
        # Set up work directory: create independent directory for each instance to avoid file conflicts
        if config and 'work_dir' in config:
            # If work_dir is specified in config, use it as base directory and create subdirectory per instance
            # This ensures each instance has independent workspace to avoid conflicts during parallel execution
            base_dir = config['work_dir']
            self.work_dir = os.path.join(base_dir, self.instance_id)
        else:
            # Otherwise use default directory based on instance_id
            default_base_dir = '/tmp/code_interpreter'
            self.work_dir = os.path.join(default_base_dir, self.instance_id)
        
        os.makedirs(self.work_dir, exist_ok=True)
        
        # Output detection timeout (to detect if kernel is stuck)
        # If no valid output (text/image/error) within this time, consider it timeout
        # This is the only timeout mechanism, simpler and stricter
        self.output_timeout = config.get('output_timeout', 30) if config else 30  # default 30 seconds
        # If set to None or 0, considered disabled (not recommended, may cause infinite wait)
        if self.output_timeout is None or self.output_timeout == 0:
            self.output_timeout = None
            # Raise exception immediately instead of waiting until execution
            raise ValueError("output_timeout must be set and > 0. It is required for timeout detection.")
    
    def _is_kernel_file(self, filename: str) -> bool:
        """Check if a file is kernel-related and should not be deleted."""
        return filename.startswith(('kernel_connection_file_', 'launch_kernel_'))
    
    def _is_image_file(self, filename: str) -> bool:
        """Check if a file is an image based on extension."""
        return filename.lower().endswith(IMAGE_EXTENSIONS)
    
    def _get_kernel_id(self) -> str:
        """Generate kernel ID for current instance, process and thread."""
        import threading
        thread_id = threading.get_ident()
        return f'{self.instance_id}_{os.getpid()}_{thread_id}'
    
    def _process_image_output(self, msg: dict, served_images: set, tool_image_counter: int) -> tuple:
        """
        Process image output from kernel message.
        
        Args:
            msg: Kernel message containing image data
            served_images: Set to track served image filenames
            tool_image_counter: Current counter for tool images
            
        Returns:
            Tuple of (image_markdown, updated_counter)
        """
        data = msg.get('content', {}).get('data', {})
        if 'image/png' in data:
            image_b64 = data['image/png']
            image_url = self._serve_image(image_b64)
            if os.path.isfile(image_url):
                served_images.add(os.path.basename(image_url))
            tool_image_counter += 1
            return f'![tool_image_{tool_image_counter}]({image_url})', tool_image_counter
        return '', tool_image_counter
    
    def _clean_work_dir(self, input_image_files: Set[str] = None):
        """
        Clean input image files in work directory while preserving user-saved images.
        
        Cleaning strategy:
        1. Preserve kernel-related files (kernel_connection_file_*, launch_kernel_*)
        2. Only delete input image files (passed via image_map, will be re-injected in this call)
        3. Preserve user-saved image files (for cross-step access)
        4. Delete other user-generated files (.py, .txt, .csv, etc., excluding kernel files)
        """
        if not os.path.exists(self.work_dir):
            return
        
        # Track input image filenames (passed via image_map)
        input_files = input_image_files if input_image_files else set()
        
        try:
            for f in os.listdir(self.work_dir):
                file_path = os.path.join(self.work_dir, f)
                
                # Skip kernel-related files
                if self._is_kernel_file(f):
                    continue
                
                # Only delete input image files, preserve user-saved images
                if self._is_image_file(f):
                    if f in input_files:
                        # This is an input image, can be deleted (will be re-injected in this call)
                        try:
                            os.remove(file_path)
                        except OSError:
                            pass
                    # else: user-saved image, keep it
                
                # Delete other user-generated files (but preserve kernel files)
                if f.endswith(('.py', '.txt', '.csv', '.json')) and not f.startswith('kernel_'):
                    if 'kernel' not in f.lower() and 'connection' not in f.lower():
                        try:
                            os.remove(file_path)
                        except OSError:
                            pass
        except Exception as e:
            # Cleaning failure should not affect code execution
            print(f"[CodeInterpreter] Warning: Failed to clean work_dir: {e}")
    
    def call(self, params, image_map: Dict[str, Image.Image] = None, **kwargs):
        """
        Execute Python code, optionally process incoming images

        Args:
            params: Dictionary or string containing code
            image_map (Dict[str, Image.Image], optional): Mapping of image identifiers to PIL Image objects.
            **kwargs: Additional parameters
            
        Returns:
            Code execution result
        """
        # Parse parameters
        if isinstance(params, str):
            try:
                params = json.loads(params)
            except json.JSONDecodeError:
                params = {'code': params}
        code = params.get('code', '') if isinstance(params, dict) else ''
        
        # Extract code (handle markdown code blocks)
        triple_match = re.search(r'```[^\n]*\n(.+?)```', code, re.DOTALL)
        if triple_match:
            code = triple_match.group(1)
        
        if not code.strip():
            return 'Error: No code provided'
        
        # Record input image filenames (for cleanup identification)
        # Support multiple image formats
        input_image_files = set()
        if image_map:
            for name in image_map.keys():
                var_name = re.sub(r'[^a-zA-Z0-9_]', '', name)
                if var_name:
                    # Record all possible extensions (actually saved as PNG, but compatible with other formats)
                    for ext in IMAGE_EXTENSIONS:
                        input_image_files.add(f'{var_name}{ext}')
                # Support both 'original_image' and 'original_image_*' for alias
                if name == 'original_image' or name.startswith('original_image_'):
                    # Alias files also support multiple formats
                    for ext in IMAGE_EXTENSIONS:
                        input_image_files.add(f'image{ext}')
        
        # Clean work directory: only delete input images, preserve user-saved images
        self._clean_work_dir(input_image_files)
        
        # Count existing tool_image_* for image naming
        tool_image_count = 0
        if image_map:
            tool_image_count = len([k for k in image_map.keys() if k.startswith('tool_image_')])
        
        # Image processing and code injection
        image_load_code = ''
        if image_map:
            image_load_code += 'from PIL import Image\n'
            # Sort image names to ensure consistent ordering (original_image first, then original_image_1, etc.)
            sorted_image_items = sorted(image_map.items(), key=lambda x: (
                0 if x[0] == 'original_image' else (1 if x[0].startswith('original_image_') else 2),
                x[0]
            ))
            
            for name, img in sorted_image_items:
                # Clean variable name to ensure validity
                var_name = re.sub(r'[^a-zA-Z0-9_]', '', name)
                if not var_name:
                    continue
                
                # Save image to work directory
                file_path = os.path.join(self.work_dir, f'{var_name}.png')
                img.save(file_path)
                
                # Create loading code
                image_load_code += f"{var_name} = Image.open('{file_path}')\n"
            
            # If image_map has original_image, automatically create image.png alias
            # This allows user code to use 'image.png' normally
            # Also create image variable as alias for original_image (backward compatibility)
            if 'original_image' in image_map:
                image_alias_path = os.path.join(self.work_dir, 'image.png')
                image_map['original_image'].save(image_alias_path)
                # Create image variable as alias for original_image
                image_load_code += "image = original_image\n"
        
        # Merge image loading code with user code
        full_code = image_load_code + code

        # Safety check
        if 'exit' in code.lower():
            return 'Error: Using exit() is not allowed'
        
        # Syntax check
        try:
            ast.parse(code)
        except SyntaxError as e:
            return f'SyntaxError: {e}'
        
        # After saving input images, record file list before execution (to detect newly generated images)
        # This excludes input images and previously preserved user-saved files
        existing_image_files = set()
        if os.path.exists(self.work_dir):
            for f in os.listdir(self.work_dir):
                if self._is_image_file(f) and not self._is_kernel_file(f):
                    existing_image_files.add(f)
        
        print(f"[CodeInterpreter] Executing code:\n{full_code[:300]}...")
        
        try:
            # Get or create kernel
            # Include thread ID to ensure isolation in parallel execution (multi-rollout scenarios)
            kernel_id = self._get_kernel_id()
            self._registered_kernel_ids.add(kernel_id)
            
            kc = self._acquire_kernel(kernel_id)
            
            # Execute code
            full_code += '\n\n'  # Ensure code ends with a newline
            
            # Calculate execution timeout: use output timeout * 2 as fallback
            # Mainly rely on output detection timeout to detect deadlocks, but execute_with_timeout needs a timeout parameter
            # Use output timeout * 2 as fallback, ensure even if output detection fails will not wait indefinitely
            # output_timeout has already been validated in __init__, so it should not be None here
            if self.output_timeout is None or self.output_timeout <= 0:
                raise ValueError("output_timeout must be set and > 0. It is required for timeout detection.")
            execution_timeout = self.output_timeout * 2
            
            result = execute_with_timeout(
                self._execute_code, 
                execution_timeout, 
                kc=kc, 
                code=full_code,
                existing_image_files=existing_image_files,
                kernel_id=kernel_id,
                tool_image_start_count=tool_image_count
            )
            
            # --- Kernel cleanup decision ---
            # After execution, determine if the kernel is corrupted and needs to be
            # killed + removed from cache so the next call creates a fresh one.
            #
            # skip_client_ops: when the background thread may still be alive and holding
            # the ZMQ socket, we MUST NOT touch kc from the main thread (no interrupt(),
            # no shutdown()), otherwise we get concurrent socket access which corrupts
            # ZMQ framing → ValueError: DELIM not in msg_list.
            _needs_cleanup = False
            _skip_client = False
            
            if isinstance(result, str):
                if result.startswith('TimeoutError: Function execution timed out'):
                    # execute_with_timeout itself timed out; background thread is still alive
                    _needs_cleanup = True
                    _skip_client = True
                elif result.startswith('Fatal Error:'):
                    # Uncaught exception bubbled up to execute_with_timeout, thread is dead
                    _needs_cleanup = True
                elif any(marker in result for marker in (
                    'No output received',
                    'Kernel communication may be interrupted',
                    'Kernel is not ready',
                    'Failed to execute code',
                    'code interpreter encountered an unexpected error',
                )):
                    # _execute_code returned normally with an error; thread is dead
                    _needs_cleanup = True
            
            # Even if no error markers, check if kernel process died unexpectedly
            if not _needs_cleanup:
                try:
                    with _KERNEL_LOCK:
                        proc = _MISC_SUBPROCESSES.get(kernel_id)
                        if proc is not None and proc.poll() is not None:
                            _needs_cleanup = True
                            print(f"[CodeInterpreter] Kernel process found dead (exit code: {proc.poll()})")
                except Exception:
                    pass
            
            if _needs_cleanup:
                print(f"[CodeInterpreter] Cleaning up kernel (skip_client_ops={_skip_client})")
                self._interrupt_kernel(kernel_id, kc, skip_client_ops=_skip_client)
            
            print(f"[CodeInterpreter] Execution completed")
            return result if result.strip() else 'Finished execution.'
            
        except Exception as e:
            error_msg = f"Error executing code: {str(e)}"
            print(f"[CodeInterpreter] {error_msg}")
            return error_msg
    
    def _acquire_kernel(self, kernel_id: str):
        """Get an existing kernel client or start a new one with call-level retry.

        If _start_kernel exhausts its internal retries (system overload), this
        method waits, cleans up resources, and tries once more before giving up.
        """
        with _KERNEL_LOCK:
            if kernel_id in _KERNEL_CLIENTS:
                return _KERNEL_CLIENTS[kernel_id]

        _CALL_LEVEL_MAX_ATTEMPTS = 2
        last_err = None
        for outer in range(1, _CALL_LEVEL_MAX_ATTEMPTS + 1):
            try:
                new_kc, new_subproc = self._start_kernel(kernel_id)
            except Exception as e:
                last_err = e
                if outer < _CALL_LEVEL_MAX_ATTEMPTS:
                    self._kill_orphan_kernels()
                    cooldown = 15
                    print(f"[CodeInterpreter] Kernel startup failed at call level "
                          f"(attempt {outer}/{_CALL_LEVEL_MAX_ATTEMPTS}), "
                          f"cooling down {cooldown}s before final retry...")
                    time.sleep(cooldown)
                    continue
                print(f"[CodeInterpreter] ERROR - Failed to start kernel {kernel_id} "
                      f"after {_CALL_LEVEL_MAX_ATTEMPTS} call-level attempts: {e}")
                import traceback
                traceback.print_exc()
                raise

            with _KERNEL_LOCK:
                if kernel_id not in _KERNEL_CLIENTS:
                    _KERNEL_CLIENTS[kernel_id] = new_kc
                    _MISC_SUBPROCESSES[kernel_id] = new_subproc
                    return new_kc
                else:
                    kc = _KERNEL_CLIENTS[kernel_id]
                    try:
                        new_kc.shutdown()
                        new_subproc.terminate()
                    except Exception:
                        pass
                    return kc

        raise RuntimeError(f"Failed to acquire kernel after all attempts: {last_err}")

    def _start_kernel(self, kernel_id: str, max_retries: int = 5):
        """Start a Jupyter kernel with retry logic and concurrency control.
        
        Uses a global semaphore (_KERNEL_STARTUP_SEMAPHORE) to serialize kernel
        startups across threads, preventing system overload when many threads
        try to launch kernels simultaneously.
        """
        self._enforce_kernel_limit(current_kernel_id=kernel_id)
        
        connection_file = os.path.join(self.work_dir, f'kernel_connection_file_{kernel_id}.json')
        launch_kernel_script = os.path.join(self.work_dir, f'launch_kernel_{kernel_id}.py')
        
        with open(launch_kernel_script, 'w') as fout:
            fout.write(LAUNCH_KERNEL_PY)
        
        last_error = None
        for attempt in range(1, max_retries + 1):
            if os.path.exists(connection_file):
                os.remove(connection_file)
            
            kernel_process = None
            kc = None
            
            # Acquire semaphore so only one thread launches a kernel at a time.
            # This prevents N concurrent Popen + wait_for_ready bursts that
            # saturate the CPU and cause all of them to time out.
            sem_wait_start = time.time()
            print(f"[CodeInterpreter] Waiting for kernel startup semaphore (attempt {attempt}/{max_retries})...")
            _KERNEL_STARTUP_SEMAPHORE.acquire()
            sem_wait_elapsed = time.time() - sem_wait_start
            
            kernel_stderr_path = os.path.join(self.work_dir, f'kernel_stderr_{kernel_id}.log')
            kernel_stderr_file = None
            try:
                kernel_stderr_file = open(kernel_stderr_path, 'w')
                active_count = len([p for p in _MISC_SUBPROCESSES.values() if p.poll() is None])
                print(f"[CodeInterpreter] Starting kernel (attempt {attempt}/{max_retries}, "
                      f"active kernels: {active_count}, sem_wait: {sem_wait_elapsed:.1f}s)")
                
                try:
                    kernel_process = subprocess.Popen(
                        [
                            sys.executable,
                            os.path.abspath(launch_kernel_script),
                            '--IPKernelApp.connection_file',
                            os.path.abspath(connection_file),
                            '--matplotlib=inline',
                            '--quiet',
                        ],
                        cwd=os.path.abspath(self.work_dir),
                        stderr=kernel_stderr_file,
                    )
                except Exception as e:
                    print(f"[CodeInterpreter] ERROR - Failed to start kernel process (attempt {attempt}/{max_retries}): {e}")
                    kernel_stderr_file.close()
                    last_error = e
                    continue
                
                # Wait for connection file creation
                max_wait = KERNEL_INIT_MAX_WAIT
                wait_time = 0
                while not os.path.isfile(connection_file) and wait_time < max_wait:
                    time.sleep(KERNEL_INIT_POLL_INTERVAL)
                    wait_time += KERNEL_INIT_POLL_INTERVAL
                
                if not os.path.isfile(connection_file):
                    print(f"[CodeInterpreter] ERROR - Connection file not created after {max_wait}s (attempt {attempt}/{max_retries})")
                    self._cleanup_failed_kernel(kernel_process, kc, kernel_stderr_file, kernel_stderr_path)
                    last_error = RuntimeError(f"Kernel connection file not created after {max_wait}s")
                    continue
                
                # Wait for file to be readable
                file_ready = False
                while wait_time < max_wait:
                    try:
                        with open(connection_file, 'r') as fp:
                            json.load(fp)
                        file_ready = True
                        break
                    except (json.JSONDecodeError, IOError):
                        time.sleep(KERNEL_INIT_POLL_INTERVAL)
                        wait_time += KERNEL_INIT_POLL_INTERVAL
                
                if not file_ready:
                    print(f"[CodeInterpreter] ERROR - Connection file not readable after {max_wait}s (attempt {attempt}/{max_retries})")
                    self._cleanup_failed_kernel(kernel_process, kc, kernel_stderr_file, kernel_stderr_path)
                    last_error = RuntimeError(f"Connection file not readable after {max_wait}s")
                    continue
                
                try:
                    kc = BlockingKernelClient(connection_file=connection_file)
                except Exception as e:
                    print(f"[CodeInterpreter] ERROR - Failed to create BlockingKernelClient (attempt {attempt}/{max_retries}): {e}")
                    self._cleanup_failed_kernel(kernel_process, kc, kernel_stderr_file, kernel_stderr_path)
                    last_error = e
                    continue
                
                try:
                    kc.load_connection_file()
                    kc.start_channels()
                    ready_timeout = 60 + (attempt - 1) * 30
                    kc.wait_for_ready(timeout=ready_timeout)
                    print(f"[CodeInterpreter] Kernel started successfully (attempt {attempt}/{max_retries})")
                    with _KERNEL_LOCK:
                        _KERNEL_CREATION_TIME[kernel_id] = time.time()
                    try:
                        kernel_stderr_file.close()
                    except Exception:
                        pass
                    return kc, kernel_process
                except Exception as e:
                    exit_code = kernel_process.poll()
                    if exit_code is not None:
                        print(f"[CodeInterpreter] ERROR - Kernel process already dead (exit code: {exit_code}), attempt {attempt}/{max_retries}")
                    else:
                        print(f"[CodeInterpreter] ERROR - Kernel process alive but not responding, attempt {attempt}/{max_retries}")
                    try:
                        kernel_stderr_file.close()
                        with open(kernel_stderr_path, 'r') as f:
                            stderr_content = f.read().strip()
                        if stderr_content:
                            print(f"[CodeInterpreter] Kernel stderr output:\n{stderr_content[-2000:]}")
                        else:
                            print(f"[CodeInterpreter] Kernel stderr output: (empty)")
                    except Exception:
                        pass
                    
                    self._cleanup_failed_kernel(kernel_process, kc, kernel_stderr_file, kernel_stderr_path)
                    last_error = e
                    
                    if attempt < max_retries:
                        self._kill_orphan_kernels()
                        wait_before_retry = 5 * (2 ** (attempt - 1))
                        print(f"[CodeInterpreter] Retrying kernel startup in {wait_before_retry}s...")
                        time.sleep(wait_before_retry)
                    continue
            finally:
                _KERNEL_STARTUP_SEMAPHORE.release()
        
        raise RuntimeError(f"Failed to start kernel after {max_retries} attempts: {last_error}")
    
    def _enforce_kernel_limit(self, current_kernel_id: str = None):
        """Evict the oldest kernels when total active count reaches _MAX_ACTIVE_KERNELS.
        
        This prevents unbounded accumulation of kernel processes from completed
        rollouts that were never explicitly cleaned up.
        """
        with _KERNEL_LOCK:
            active = {
                kid: proc for kid, proc in _MISC_SUBPROCESSES.items()
                if proc.poll() is None and kid != current_kernel_id
            }
            excess = len(active) - _MAX_ACTIVE_KERNELS + 1  # free 1 slot for new kernel
            if excess <= 0:
                return
            
            sorted_ids = sorted(active.keys(), key=lambda k: _KERNEL_CREATION_TIME.get(k, 0))
            evicted = 0
            for kid in sorted_ids[:excess]:
                proc = _MISC_SUBPROCESSES.pop(kid, None)
                client = _KERNEL_CLIENTS.pop(kid, None)
                _KERNEL_CREATION_TIME.pop(kid, None)
                if client is not None:
                    try:
                        client.shutdown()
                    except Exception:
                        pass
                if proc is not None and proc.poll() is None:
                    try:
                        proc.kill()
                        proc.wait(timeout=5)
                    except Exception:
                        pass
                evicted += 1
            if evicted:
                print(f"[CodeInterpreter] Evicted {evicted} oldest kernel(s) to stay under limit ({_MAX_ACTIVE_KERNELS})")
    
    def _kill_orphan_kernels(self):
        """Kill kernel processes tracked in _MISC_SUBPROCESSES that are already dead
        or no longer have a corresponding client, freeing system resources before
        a retry attempt."""
        cleaned = 0
        with _KERNEL_LOCK:
            dead_ids = [
                kid for kid, proc in _MISC_SUBPROCESSES.items()
                if proc.poll() is not None or kid not in _KERNEL_CLIENTS
            ]
            for kid in dead_ids:
                proc = _MISC_SUBPROCESSES.pop(kid, None)
                client = _KERNEL_CLIENTS.pop(kid, None)
                _KERNEL_CREATION_TIME.pop(kid, None)
                if proc is not None and proc.poll() is None:
                    try:
                        proc.kill()
                        proc.wait(timeout=5)
                    except Exception:
                        pass
                elif proc is not None:
                    try:
                        proc.wait(timeout=1)
                    except Exception:
                        pass
                if client is not None:
                    try:
                        client.shutdown()
                    except Exception:
                        pass
                cleaned += 1
        if cleaned:
            print(f"[CodeInterpreter] Cleaned up {cleaned} orphan kernel(s)")
    
    def _cleanup_failed_kernel(self, kernel_process, kc, stderr_file, stderr_path):
        """Clean up resources from a failed kernel startup attempt."""
        try:
            stderr_file.close()
        except Exception:
            pass
        if kc is not None:
            try:
                kc.stop_channels()
            except Exception:
                pass
        if kernel_process is not None:
            try:
                kernel_process.terminate()
                kernel_process.wait(timeout=5)
            except Exception:
                try:
                    kernel_process.kill()
                except Exception:
                    pass
    
    def _is_valid_output(self, msg_type: str, msg: dict) -> bool:
        """
        Determine if a message is valid output (for output detection timeout).
        
        Args:
            msg_type: Message type
            msg: Message content
            
        Returns:
            bool: Whether it's valid output
        """
        if msg_type == 'status':
            # Status messages don't count as valid output (just state updates)
            return False
        elif msg_type == 'execute_result':
            # Check if there's actual content
            data = msg.get('content', {}).get('data', {})
            if 'image/png' in data:
                return True  # Images count as valid output
            text = data.get('text/plain', '')
            return bool(text and text.strip())  # Only counts if there's text content
        elif msg_type == 'display_data':
            # Check if there's image or text
            data = msg.get('content', {}).get('data', {})
            if 'image/png' in data:
                return True
            text = data.get('text/plain', '')
            return bool(text and text.strip())
        elif msg_type == 'stream':
            # Stream output counts as valid output
            text = msg.get('content', {}).get('text', '')
            return bool(text and text.strip())
        elif msg_type == 'error':
            # Errors also count as valid output (at least we know something went wrong)
            return True
        return False
    
    def _interrupt_kernel(self, kernel_id, kc, skip_client_ops=False):
        """Interrupt kernel execution.
        
        Attempts multiple methods to interrupt kernel:
        1. Send interrupt message via shell channel (if supported, and skip_client_ops=False)
        2. Send SIGINT signal to kernel process
        3. If SIGINT fails, send SIGKILL to forcefully terminate
        4. Clean up kernel references in global dictionaries
        
        Args:
            kernel_id: Kernel ID
            kc: Kernel client
            skip_client_ops: If True, skip all operations on kc (interrupt/shutdown)
                to avoid ZMQ socket contention when a background thread may still
                be using the same client. Only use process-level signals and dict cleanup.
        """
        if kernel_id is None:
            print(f"[CodeInterpreter] Cannot interrupt kernel: kernel_id is None")
            return
        
        # Method 1: Try sending interrupt message via shell channel
        if not skip_client_ops:
            try:
                if hasattr(kc, 'interrupt'):
                    kc.interrupt()
                    print(f"[CodeInterpreter] Kernel interrupt via interrupt() method")
            except Exception as e:
                print(f"[CodeInterpreter] Failed to interrupt kernel via client: {e}")
        else:
            print(f"[CodeInterpreter] Skipping client operations (background thread may still hold the socket)")
        
        # Method 2: Send SIGINT signal to kernel process, if ineffective use SIGKILL
        kernel_process = None
        try:
            with _KERNEL_LOCK:
                if kernel_id in _MISC_SUBPROCESSES:
                    kernel_process = _MISC_SUBPROCESSES[kernel_id]
        except Exception as e:
            print(f"[CodeInterpreter] Failed to get kernel process: {e}")
        
        if kernel_process and kernel_process.poll() is None:
            try:
                kernel_process.send_signal(signal.SIGINT)
                print(f"[CodeInterpreter] Kernel interrupt via SIGINT signal sent to process")
                
                time.sleep(1.0)
                
                if kernel_process.poll() is None:
                    print(f"[CodeInterpreter] Kernel process not responding to SIGINT, sending SIGKILL")
                    kernel_process.kill()
                    try:
                        kernel_process.wait(timeout=5)
                        print(f"[CodeInterpreter] Kernel process terminated via SIGKILL")
                    except subprocess.TimeoutExpired:
                        print(f"[CodeInterpreter] WARNING: Kernel process still running after SIGKILL")
                else:
                    print(f"[CodeInterpreter] Kernel process terminated via SIGINT")
                    
            except Exception as e:
                print(f"[CodeInterpreter] Failed to terminate kernel process: {e}")
        
        # Method 3: Clean kernel references in global dictionary (ensure kernel is recreated on next call)
        try:
            with _KERNEL_LOCK:
                if kernel_id in _KERNEL_CLIENTS:
                    if not skip_client_ops:
                        try:
                            _KERNEL_CLIENTS[kernel_id].shutdown()
                        except:
                            pass
                    del _KERNEL_CLIENTS[kernel_id]
                    print(f"[CodeInterpreter] Kernel client removed from cache")
                if kernel_id in _MISC_SUBPROCESSES:
                    del _MISC_SUBPROCESSES[kernel_id]
                    print(f"[CodeInterpreter] Kernel process removed from cache")
                _KERNEL_CREATION_TIME.pop(kernel_id, None)
        except Exception as e:
            print(f"[CodeInterpreter] Failed to cleanup kernel references: {e}")
    
    def _execute_code(self, kc, code: str, existing_image_files=None, _stop_event=None, kernel_id=None, tool_image_start_count=0) -> str:
        try:
            return self._execute_code_inner(kc, code, existing_image_files, _stop_event, kernel_id, tool_image_start_count)
        finally:
            if _stop_event and _stop_event.is_set():
                try:
                    kc.stop_channels()
                    print("[CodeInterpreter] Background thread stopped dangling ZMQ channels to prevent leaks")
                except Exception:
                    pass

    def _execute_code_inner(self, kc, code: str, existing_image_files=None, _stop_event=None, kernel_id=None, tool_image_start_count=0) -> str:
        """Execute code and get results (simplified version, using signal mechanism for timeout).
        
        Args:
            kc: Kernel client
            code: Code to execute
            existing_image_files: Set of image files that exist before execution (if None, will scan directory)
            _stop_event: threading.Event object for graceful interruption
            kernel_id: Kernel ID for interrupting kernel process
            tool_image_start_count: Number of existing tool_image_* for image naming
        """
        # Use provided existing_image_files, if not provided scan directory
        if existing_image_files is None:
            existing_image_files = set()
            if os.path.exists(self.work_dir):
                for f in os.listdir(self.work_dir):
                    if f.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
                        if f.startswith('kernel_') or f.startswith('launch_kernel_'):
                            continue
                        existing_image_files.add(f)
        
        served_images = set()  # Record image files saved through _serve_image
        
        try:
            kc.wait_for_ready(timeout=5)
        except Exception as e:
            return f'Fatal Error: Kernel is not ready. {str(e)}'
        
        # Execute code
        try:
            kc.execute(code)
        except Exception as e:
            return f'Fatal Error: Failed to execute code. {str(e)}'
        
        result = ''
        tool_image_counter = tool_image_start_count
        start_time = time.time()
        
        # Output detection timeout: track last valid output time
        last_output_time = start_time  # Initialize to start time
        output_timeout_enabled = self.output_timeout is not None and self.output_timeout > 0
        
        if not output_timeout_enabled:
            raise ValueError("output_timeout must be set and > 0. It is required for timeout detection.")
        
        while True:
            text = ''
            image = ''
            finished = False
            msg_type = 'error'
            
            # ========== Unified checkpoint: Timeout and interrupt check ==========
            # NOTE: kernel cleanup is NOT done here - it is handled by call() after
            # execute_with_timeout returns, to avoid concurrent ZMQ socket access
            # between the background thread (_execute_code) and the main thread (call).
            
            # 1. Check stop flag (from execute_with_timeout)
            if _stop_event and _stop_event.is_set():
                print(f"[CodeInterpreter] Stop event detected, exiting execution loop")
                result += '\n\nerror:\n\n```\nTimeoutError: Execution was interrupted due to timeout.\n```'
                break
            
            # 2. Check output detection timeout
            time_since_last_output = time.time() - last_output_time
            
            if time_since_last_output >= self.output_timeout:
                print(f"[CodeInterpreter] Output timeout detected: {time_since_last_output:.1f}s without valid output")
                result += f'\n\nerror:\n\n```\nTimeoutError: No output received for {time_since_last_output:.1f} seconds (output_timeout: {self.output_timeout}s). The kernel may be unresponsive or stuck.\n```'
                break
            
            try:
                # Set single wait time: limit to 10 seconds to avoid long blocking
                # Calculate how much time until output timeout
                time_until_output_timeout = self.output_timeout - time_since_last_output
                # Take minimum: time until output timeout, 10 seconds
                single_wait_timeout = min(max(1, time_until_output_timeout), 10)
                
                # Check stop flag again before waiting for message (avoid long blocking)
                if _stop_event and _stop_event.is_set():
                    continue
                
                # Block and wait for message, but with timeout
                msg = kc.get_iopub_msg(timeout=single_wait_timeout)
                msg_type = msg.get('msg_type', 'unknown')
                
                # Check if it's valid output and update last output time
                if output_timeout_enabled and self._is_valid_output(msg_type, msg):
                    last_output_time = time.time()
                    print(f"[CodeInterpreter] Received valid output (type: {msg_type}), resetting output timeout timer")
                
                # Handle various message types
                if msg_type == 'status':
                    execution_state = msg.get('content', {}).get('execution_state', 'unknown')
                    if execution_state == 'idle':
                        finished = True
                    # status message is not valid output, do not reset output timer
                        
                elif msg_type == 'execute_result':
                    content_data = msg.get('content', {}).get('data', {})
                    text = content_data.get('text/plain', '')
                    image, tool_image_counter = self._process_image_output(msg, served_images, tool_image_counter)
                        
                elif msg_type == 'display_data':
                    image, tool_image_counter = self._process_image_output(msg, served_images, tool_image_counter)
                    if not image:  # If no image, check for text
                        content_data = msg.get('content', {}).get('data', {})
                        text = content_data.get('text/plain', '')
                        
                elif msg_type == 'stream':
                    msg_type = msg.get('content', {}).get('name', 'stdout')  # stdout or stderr
                    text = msg.get('content', {}).get('text', '')
                    
                elif msg_type == 'error':
                    traceback_list = msg.get('content', {}).get('traceback', [])
                    error_text = _escape_ansi('\n'.join(traceback_list))
                    text = error_text
                    
            except queue.Empty:
                # Message queue timeout: continue loop, let unified checkpoint handle output timeout
                # If just single wait timeout but not yet output timeout, continue loop
                # If already output timeout, unified checkpoint will handle it on next iteration
                continue
                
            except (ValueError, ConnectionError, OSError) as e:
                # ZMQ/connection related errors - always append error info so call()
                # can detect kernel corruption and clean up, even with partial results
                text = f'Error: {str(e)}. Kernel communication may be interrupted.'
                finished = True
                    
            except BaseException as e:
                # Catch all other exceptions
                error_msg = str(e)
                error_type = type(e).__name__
                print(f"[CodeInterpreter] FATAL - {error_type}: {error_msg}")
                import traceback
                traceback.print_exc()
                
                text = f'Fatal Error ({error_type}): {error_msg}. The code interpreter encountered an unexpected error.'
                finished = True
            
            if text:
                result += f'\n\n{msg_type}:\n\n```\n{text}\n```'
            if image:
                result += f'\n\n{image}'
            if finished:
                break
        
        # Skip image scanning on abnormal exits (timeout, communication error, etc.)
        _ABNORMAL_EXIT_MARKERS = (
            'No output received',
            'Execution was interrupted due to timeout',
            'Kernel communication may be interrupted',
            'code interpreter encountered an unexpected error',
        )
        if result and any(m in result for m in _ABNORMAL_EXIT_MARKERS):
            print(f"[CodeInterpreter] Skipping image scan due to abnormal exit, returning immediately")
            return result.lstrip('\n')
        
        # After code execution, scan work directory to find newly generated image files
        # Since each instance has a separate work directory, just compare file names
        new_image_files = []
        if os.path.exists(self.work_dir):
            for f in os.listdir(self.work_dir):
                if self._is_image_file(f):
                    # Exclude known files (like kernel connection files, launch scripts, etc.)
                    if self._is_kernel_file(f):
                        continue
                    # Exclude images already returned via execute_result/display_data (avoid duplicates)
                    if f in served_images:
                        continue
                    # Only add newly generated files
                    if f not in existing_image_files:
                        new_image_files.append(f)
        
        # Add newly generated image files to result
        for img_file in sorted(new_image_files):
            image_path = os.path.join(self.work_dir, img_file)
            if os.path.exists(image_path):
                tool_image_counter += 1
                # Use absolute path to ensure api_processors.py can find file correctly
                abs_image_path = os.path.abspath(image_path)
                result += f'\n\n![tool_image_{tool_image_counter}]({abs_image_path})'
                print(f"[CodeInterpreter] Auto-detected saved image: {img_file} -> {abs_image_path}")
        
        return result.lstrip('\n')
    
    def _serve_image(self, image_base64: str) -> str:
        """Save image and return path"""
        image_file = f'{uuid.uuid4()}.png'
        local_image_file = os.path.join(self.work_dir, image_file)
        
        png_bytes = base64.b64decode(image_base64)
        bytes_io = io.BytesIO(png_bytes)
        Image.open(bytes_io).save(local_image_file, 'png')
        
        return local_image_file
    
    def cleanup(self):
        """Explicitly release all kernels registered by this instance.
        
        Call this when the CodeInterpreter is no longer needed (e.g. after a
        rollout finishes) instead of relying on __del__ / GC.
        """
        kernel_ids = list(getattr(self, '_registered_kernel_ids', ()))
        for kernel_id in kernel_ids:
            try:
                self._force_cleanup_kernel(kernel_id)
            except Exception:
                pass
        self._registered_kernel_ids.clear()

    def _force_cleanup_kernel(self, kernel_id: str):
        """Kill a kernel process and remove it from global dicts.
        
        Uses SIGTERM → wait → SIGKILL escalation to ensure the process is
        actually dead, unlike the old __del__ which only called terminate().
        """
        with _KERNEL_LOCK:
            client = _KERNEL_CLIENTS.pop(kernel_id, None)
            proc = _MISC_SUBPROCESSES.pop(kernel_id, None)
            _KERNEL_CREATION_TIME.pop(kernel_id, None)

        if client is not None:
            try:
                client.shutdown()
            except Exception:
                pass

        if proc is not None and proc.poll() is None:
            try:
                proc.terminate()
                try:
                    proc.wait(timeout=3)
                except subprocess.TimeoutExpired:
                    proc.kill()
                    proc.wait(timeout=5)
            except Exception:
                pass

    def __del__(self):
        """Best-effort cleanup via GC.  Prefer calling cleanup() explicitly."""
        try:
            self.cleanup()
        except Exception:
            pass


def _escape_ansi(line: str) -> str:
    """Remove ANSI escape sequences"""
    ansi_escape = re.compile(r'(?:\x1B[@-_]|[\x80-\x9F])[0-?]*[ -/]*[@-~]')
    return ansi_escape.sub('', line)


# Event loop policy (for compatibility)
if sys.platform == 'win32' and hasattr(asyncio, 'WindowsSelectorEventLoopPolicy'):
    _BasePolicy = asyncio.WindowsSelectorEventLoopPolicy
else:
    _BasePolicy = asyncio.DefaultEventLoopPolicy


class AnyThreadEventLoopPolicy(_BasePolicy):
    """Event loop policy that allows creating event loops in any thread."""
    
    def get_event_loop(self) -> asyncio.AbstractEventLoop:
        try:
            return super().get_event_loop()
        except RuntimeError:
            loop = self.new_event_loop()
            self.set_event_loop(loop)
            return loop


# Set event loop policy once at module level instead of per-thread in _start_kernel.
# asyncio.set_event_loop_policy() modifies process-global state; calling it from
# multiple threads concurrently is a race condition.
try:
    asyncio.set_event_loop_policy(AnyThreadEventLoopPolicy())
except Exception:
    pass

