...
## Guiding Logic & Constraints
Adhere to these rules to prevent calculation errors. These apply universally across estimation strategies.

| Constraint | Logic | Action |
| :--- | :--- | :--- |
| **Geometric Upper Bound** | Target Area ≤ Bounding Box Area. | Discard any option violating this immediately. Select highest valid option below bound. |
| **Quantitative Trust** | Do not arbitrarily reduce tool outputs if they align closely with options. | If `Tool_Output ≈ Option` (within tolerance), trust metric unless mask is visibly broken. |
| **Footprint vs. Pixels** | Distinguish between "visible pixels" and "total object extent". | If segmentation < Options significantly, switch to geometric modeling (bounding box + fill factor). |
| **Context Separation** | Exclude attached accessories unless specified. | Apply strict ROI masking if segmentation merges subject + background clutter. |
| **Tolerance Margin** | Manual bounding boxes have error margins. | If `BB_Ratio ≈ Option` within tolerance, retain option. Measurement error likely exists. |

### Heuristic Adjustments
Apply calibration factors based on object characteristics to refine raw segmentation data:
...
-   **High Estimate Validation:** Do not immediately discard high percentage estimates as noise; verify if they capture the full object footprint including shadows/garnish.
-   **Avoid Over-Correction:** If a tool calculates a value close to an option, trust the quantitative result unless the mask clearly includes large empty spaces.
...
def segment_target_and_verify(image, method='hsv', roi_coords=None, target_params=None):
    ...
    if method == 'hsv':
        ...
        rect = (roi_coords[0], roi_coords[1], roi_coords[2]-roi_coords[0], roi_coords[3]-roi_coords[1])
...
## Common Pitfalls
-   **Color Variance:** Avoid relying solely on color thresholding as lighting changes cause false positives/negatives.
-   **Noise Inclusion:** High proportions often indicate background noise was included. Always visualize the mask.
-   **Segmentation Gaps:** Algorithms may miss dark spots or edges. If calculated value is near an option boundary, consider if the mask missed small areas.
-   **Context Merging:** Be wary of segmentation merging the subject with adjacent objects. Use tight ROIs to isolate the semantic target.
-   **Over-Correction:** Do not apply arbitrary fill factors to segmentations that claim to measure area directly. Trust high ratio matches unless masks are visibly broken.